...
import com.base.application.app.AppLifecyclesImpl
import com.example.kotlin.api.Api
...
class GlobalConfiguration : ConfigModule {
    override fun applyOptions(@NonNull context: Context, @NonNull builder: GlobalConfigModule.Builder) {
        if (!BuildConfig.DEBUG) { //Release 时, 让框架不再打印 Http 请求和响应的信息
            builder.printHttpLogLevel(RequestInterceptor.Level.NONE)
        }
        builder.baseurl(Api.API)
    ...
    override fun injectAppLifecycle(@NonNull context: Context, @NonNull lifecycles: MutableList<AppLifecycles>) {
        //AppLifecycles 中的所有方法都会在基类 Application 的对应生命周期中被调用, 所以在对应的方法中可以扩展一些自己需要的逻辑
        //可以根据不同的逻辑添加多个实现类
        lifecycles.add(AppLifecyclesImpl())
    }

    override fun injectActivityLifecycle(@NonNull context: Context, @NonNull lifecycles: MutableList<Application.ActivityLifecycleCallbacks>) {
    ...
    override fun injectFragmentLifecycle(@NonNull context: Context, @NonNull lifecycles: MutableList<FragmentManager.FragmentLifecycleCallbacks>) {
